...
resumepause:c,movecount:1,responsive:c},function(c){var e={},f;for(f in c)e[f.toLowerCase()]=c[f];return e}(ga));return this.each(function(){function B(d,p){l=0;for(b in O)a[l]=O[b],l++;z=c;J=e;q=d.children("ul");0==q.length&&d.append(q=f("<ul></ul>"));n=q.children("li");k=n.length;if(a[24]&&a[24].length>k){for(b=1;b<=a[24].length-k;b++)q.append("<li><p>"+a[34]+"</p></li>");n=q.children("li");k=n.length}C=g=0;P=k-1;m=e;Q=u=ia=c;$=[];z=c;"static"==d.css("position")&&d.css("position","relative");n.css({"float":"left",
display:"block"});a[39]=o(a[39]);a[41]--;s=a[39];a[20]||(a[39]+=a[41]);a[26]=o(a[26])||1;R=a[11]&&(!a[20]||1<a[39]);for(b=0;b<k;b++)a[15][b]=a[15][b]||b+1,a[24][b]=a[24][b]||c;for(h=0;h<k;h++)v[h]==Z&&(v[h]=[]),v[h].push(n.eq(h));if(R){for(h=a[39];1<=h;h--){var D=w(-a[39]+h-1),t=w(a[39]-h),i=n.eq(D).clone(),ma=n.eq(t).clone();v[D].push(i);v[t].push(ma);q.prepend(i).append(ma)}x=q.children("li");if(a[24])for(b=k-a[39];b<k;b++)a[24][b]&&b!=a[26]-1&&E(b,c,0,c)}a[2]=a[2]&&!a[11];q[a[6]?"height":"width"](ya);
x=q.children("li");a[38]=a[38]&&!a[42];a[42]&&f(window).on("resize focus",M).resize();F=c;if(a[0]){F=f("<span "+a[36]+"></span>");f(d)[a[3]?"after":"before"](F);if(a[13]){na=F.prepend("<ol "+a[14]+"></ol>").children();l="pages"==a[13]?s:1;for(b=0;b<k-(a[11]||"pages"==a[13]?1:s)+1;b+=l)$[b]=f("<li rel='"+(b+1)+"'><a href='#'><span>"+a[15][b]+"</span></a></li>").appendTo(na).click(function(){r(f(this).attr("rel")-1,e);return c})}a[4]&&(oa=y(a[35],"first"));a[5]&&(pa=y(a[37],"last"));a[12]&&(qa=y(a[33],
"next"),ra=y(a[32],"prev"))}if(a[25]===e)for(h=0;h<=P;h++)a[24][h]&&a[26]-1!=h&&E(h,c,0,c);l=[1,7,10,18,23];for(b in l)D=a,t=o(l[b]),i=a[o(l[b])],i=o(i)||0==i?o(i):"fast"==i?200:"normal"==i||"medium"==i?400:"slow"==i?600:400,D[t]=i;if(a[19])f(document).on("click",a[19],function(){if(b=f(this).attr("rel"))"stop"==b?(a[9]=c,clearTimeout(A),S=c):"start"==b?(A=N(a[10]),a[9]=e):"block"==b?m=c:"unblock"==b?m=e:m&&r(b==o(b)?b-1:b,e);return c});ja(x.slice(0,a[39]),e,function(){a[9]&&(A=N(a[10]));if(p)G(p,
c,c,c);else if(a[16]){b=f(window);if(h=b.hashchange)h(Y);else if(h=f.address)h.change(Y);else b.on("hashchange",Y);Y()}else G(a[26]-1,c,c,c)})}function M(){x.width(H.width()/s);T(g,0);U()}function Y(){a:{var d=location.hash.substr(1);for(h in a[15])if(a[15][h]==d)break a;h=d?g:0}J?G(h,c,c,c):h!=g&&r(h,c)}function ha(){if(a[24]&&o(a[25]))for(b in a[24])if(a[24][b]){clearTimeout(aa);aa=setTimeout(function(){a[24][b]?E(b,c,0,c):ha()},o(a[25]));break}}function N(a){S=e;return setTimeout(function(){r("next",
...
C>g?C:g;X=0;da=i;for(b=j;b<=l;b++)b<=P&&(0<=b&&a[24][b])&&(E(b,c,i,function(){G(d,f,h,e)}),X++)}for(b=n+1;b<=n+s;b++)a[24][b]&&E(b,c,0,c)}X||(!u&&!Q&&(I(n,c),V=e),u||T(g,i),q.animate({marginTop:ca(g,e),marginLeft:ca(g,c)},{queue:c,duration:i,easing:a[8],complete:za}),a[2]&&(j=a[1],!f&&!a[9]&&(j=a[17]/a[7]*a[1]),h||(j=0),u&&(j=o(0.6*a[22])),i=g,ba(i,j,c),ba(i<k-s,j,e)),J&&(a[24][n]||ha()),J=c)}}function w(a){if(0==k)return 0;for(a=o(a);0>a;)a+=k;return a%k}function o(a){return parseInt(a,10)}function ea(){L=
g;a[42]&&f(window).off("resize focus",M);F&&F.remove();z=e;f(a[19]).die("click");if(R)for(b=1;b<=a[39];b++)x.eq(b-1).add(x.eq(-b)).remove();U()}function fa(){z&&B(H,L)}var J,q,n,x,k,g,C,P,m,ia,u,Q,$,na,z,F,oa,pa,qa,ra,A,L,da,X,K,S,b,l,h,R,s,V=c,aa,v=[],H=f(this),ua=c,O=ga,a=[],ya=1E7;B(H,c);j("destroy",ea);j("init",fa);j("getOption",function(a){return O[a.toLowerCase()]});j("setOption",function(a,b){ea();O[a.toLowerCase()]=b;fa()});j("insertSlide",function(b,c,e){ea();c>k&&(c=k);b="<li>"+b+"</li>";
!c||0==c?q.prepend(b):n.eq(c-1).after(b);(c<=L||!c||0==c)&&L++;a[15].length<c&&(a[15].length=c);a[15].splice(c,0,e||o(c)+1);fa()});j("removeSlide",function(b){b--;ea();n.eq(b).remove();a[15].splice(b,1);b<L&&L--;fa()});j("goToSlide",function(a,b){r(a==o(a)?a-1:a,e,b)});j("block",function(){m=c});j("unblock",function(){m=e});j("startAuto",function(){a[9]=e;A=N(a[10])});j("stopAuto",function(){a[9]=c;clearTimeout(A);S=c});j("adjust",function(){T(g,0);U()});j("getValue",function(a){return"currentSlide"==
a?g+1:"totalSlides"==a?k:"clickable"==a?m:"destroyed"==a?z:"autoAnimation"==a?S:Z})})}})(jQuery);