...
resumepause:c,movecount:1,responsive:c},function(c){var e={},f;for(f in c)e[f.toLowerCase()]=c[f];return e}(ha));return this.each(function(){function B(d,q){k=0;for(b in R)a[k]=R[b],k++;A=c;J=e;r=d.children("ul");0==r.length&&d.append(r=f("<ul></ul>"));s=r.children("li");m=s.length;if(a[24]&&a[24].length>m){for(b=1;b<=a[24].length-m;b++)r.append("<li><p>"+a[34]+"</p></li>");s=r.children("li");m=s.length}C=g=0;S=m-1;n=e;T=x=$=c;aa=[];A=c;"static"==d.css("position")&&d.css("position","relative");s.css({"float":"left",
display:"block"});a[39]=p(a[39]);a[41]--;u=a[39];a[20]||(a[39]+=a[41]);a[26]=p(a[26])||1;(v=a[11]&&(!a[20]||1<a[39]))&&(v=[]);for(b=0;b<m;b++)a[15][b]=a[15][b]||b+1,a[24][b]=a[24][b]||c;t=[];for(h=0;h<m;h++)t[h]=[],t[h].push(s.eq(h));if(v){for(h=a[39];1<=h;h--){var K=w(-a[39]+h-1),l=w(a[39]-h),i=s.eq(K).clone();v.push(i);var ka=s.eq(l).clone();v.push(ka);t[K].push(i);t[l].push(ka);r.prepend(i).append(ka)}D=r.children("li")}a[2]=a[2]&&!a[11];r[a[6]?"height":"width"](Aa);D=r.children("li");a[38]=a[38]&&
...
h&&e.filter(":visible").fadeOut(c)));a[19]&&(b=function(){return f(this).attr("rel")==i||f(this).attr("rel")==g},d?f(a[19]).filter(b).filter(":hidden").fadeIn(c):f(a[19]).filter(b).filter(":visible").fadeOut(c))}function na(d){d=w(d)+1;if(a[13])for(b in aa)va(aa[b],d);a[19]&&va(f(a[19]),d)}function va(d,q){d.filter&&(d.filter(".current").removeClass("current").each(function(){f.isFunction(a[30])&&a[30].call(this,f(this).attr("rel"))}),d.filter(function(){k=f(this).attr("rel");if("pages"==a[13])for(b=
...
else if(a.complete)f(a).trigger("load");else if(a.complete===ja){var d=a.src;a.src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==";a.src=d}})}function U(d,b){wa=0==b?d:c;a[18]&&xa(d,b,e);a[38]&&xa(d,b,c)}function xa(a,b,e){G.ready(function(){ya(a,b,e);la(s.eq(a),c,function(){ya(a,b,e)})})}function ya(d,b,e){d=w(d);k=s.eq(d)[e?"height":"width"]();G.animate(e?{height:k}:{width:k},{queue:c,duration:b,easing:a[8]})}function V(){r.css("margin-left",da(g,c)).css("margin-top",da(g,
e))}function da(d,c){d=D.eq(d+(v?a[39]:0));return d.length?-d.position()[c?"top":"left"]:0}function Ba(){g=w(g);a[23]||na(g);V();n=e;a[16]&&$&&(window.location.hash=a[15][g]);a[9]&&($?(z(),a[40]&&(M=Q(a[40]))):M=Q(a[10]));!x&&W&&H(g,e)}function H(a,c){var a=w(a),e=f();for(b in t[a])e=e.add(t[a][b]);(c?Ca:Da)(e,a+1)}function Ca(d,c){f.isFunction(a[29])&&a[29].call(d,c)}function Da(d,c){f.isFunction(a[28])&&a[28].call(d,c)}function X(d){return"next"==d?oa(g+1+a[41],d):"prev"==d?oa(g-1-a[41],d):"first"==
d?0:"last"==d?S:oa(p(d),d)}function oa(d,c){if(a[11])return w(d);var b=m-u;return d>b?g==b&&"next"==c?0:b:0>d?0==g&&"prev"==c?b:0:d}function N(d,b,g,l){ba&&clearTimeout(ba);var i=a[24][d],h=s.eq(d),m=g===e,g=g===e?0:g;x&&!a[21]&&p(0.4*a[22]);var j=c;f.ajax({url:i,success:function(a,f,i){"image"!=i.getResponseHeader("Content-Type").substr(0,5)&&(j=e,h.html(a),za(d,g,l,b,m,c))},complete:function(){j||(image=new Image,h.html("").append(image),image.src=i,za(d,g,l,b,m,e))}});a[24][d]=c;R.ajax[d]=c}function za(d,
...
0;ea=l;for(b=i;b<i+u;b++)a[24][b]&&(N(w(b),c,l,function(){ua(d,q,e)}),O++)}else O=c;if(!O)if(n=!q,U(i,a[22]),H(i,c),a[21]){var r=e,h=0;for(b=i;b<i+u;b++)s.eq(w(b)).clone().prependTo(G).css({"z-index":"10000",position:"absolute","list-style":"none",top:a[6]?h:0,left:a[6]?0:h}).hide().fadeIn(a[22],a[8],function(){screen.fontSmoothingEnabled&&this.style.removeAttribute("filter");x=n=e;r&&(F(i,q,c,c),a[16]&&q&&(window.location.hash=a[15][g]),H(i,e),r=c);f(this).remove();x=c}),h+=G.width()/u}else{var t=
p(0.6*l),v=l-t;D.each(function(){f(this).animate({opacity:1E-4},{queue:c,duration:v,easing:a[8],complete:function(){x=n=e;F(i,c,c,c);n=!q;f(this).animate({opacity:1},{queue:c,duration:t,easing:a[8],complete:function(){screen.fontSmoothingEnabled&&this.style.removeAttribute("filter");a[16]&&q&&(window.location.hash=a[15][g]);n=e;x=c;H(i,e)}})}})})}}}function F(d,f,h,l,i){if(n&&(!A&&(d!=g||J))&&m>w(d)||l){l||(T=c);n=!f&&!a[9];$=f;C=g;g=d;a[23]&&na(g);var j=Math.sqrt(Math.abs(C-g));if(!(i||0==i))var i=
!h?0:!f&&!a[9]?p(j*a[17]):p(j*a[7]),k=w(g);if(l)i=ea,Y&&Y--;else if(a[24]){a[24][k]&&(N(k,e,J||i,2),T=e);if(!x){l=C>g?g:C;j=C>g?C:g;Y=0;ea=i;for(b=l;b<=j;b++)b<=S&&(0<=b&&a[24][b])&&(N(b,c,i,function(){F(d,f,h,e)}),Y++)}for(b=k+1;b<=k+u;b++)a[24][b]&&N(b,c,0,c)}Y||(!x&&!T&&(H(k,c),W=e),x||U(g,i),r.animate({marginTop:da(g,e),marginLeft:da(g,c)},{queue:c,duration:i,easing:a[8],complete:Ba}),a[2]&&(l=a[1],!f&&!a[9]&&(l=a[17]/a[7]*a[1]),h||(l=0),x&&(l=p(0.6*a[22])),i=g,ca(i,l,c),ca(i<m-u,l,e)),J&&(a[24][k]||
ia()),J=c)}}function w(a){if(0==m)return 0;for(a=p(a);0>a;)a+=m;return a%m}function p(a){return parseInt(a,10)}function fa(){I=g;a[42]&&f(window).off("resize focus",P);E&&E.remove();A=e;f(a[19]).die("click");if(v)for(var d=0;d<v.length;d++)v[d].remove();V()}function ga(){A&&B(G,I)}var J,r,s,D,m,g,C,S,n,$,x,T,aa,pa,A,E,qa,ra,sa,ta,M,I,ea,Y,O,ma,b,k,h,v,u,W=c,ba,t,G=f(this),wa=c,R=ha,a=[],Aa=1E7;B(G,c);j("destroy",fa);j("init",ga);j("getOption",function(a){return R[a.toLowerCase()]});j("setOption",
...
function(){a[9]=c;z()});j("adjust",function(){U(g,0);V()});j("getValue",function(a){return"currentSlide"==a?g+1:"totalSlides"==a?m:"clickable"==a?n:"destroyed"==a?A:"autoAnimation"==a?ma:ja});j("getSlide",function(a){var a=w(p(a)-1),c=f();for(b in t[a])c=c.add(t[a][b]);return c})})}})(jQuery);