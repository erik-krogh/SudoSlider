...
f(window).off("resize focus",M);F&&F.remove();z=e;f(a[19]).die("click");if(R)for(b=1;b<=a[39];b++)x.eq(b-1).add(x.eq(-b)).remove();U()}function fa(){z&&B(H,L)}var J,q,n,x,k,g,C,P,m,ia,u,Q,$,na,z,F,oa,pa,qa,ra,A,L,da,X,K,S,b,l,h,R,s,V=c,aa,v=[],H=f(this),ua=c,O=ga,a=[],ya=1E7;B(H,c);j("destroy",ea);j("init",fa);j("getOption",function(a){return O[a]});j("setOption",function(a,b){ea();O[a.toLowerCase()]=b;fa()});j("insertSlide",function(b,c,e){ea();c>k&&(c=k);b="<li>"+b+"</li>";!c||0==c?q.prepend(b):
n.eq(c-1).after(b);(c<=L||!c||0==c)&&L++;a[15].length<c&&(a[15].length=c);a[15].splice(c,0,e||o(c)+1);fa()});j("removeSlide",function(b){b--;ea();n.eq(b).remove();a[15].splice(b,1);b<L&&L--;fa()});j("goToSlide",function(a,b){r(a==o(a)?a-1:a,e,b)});j("block",function(){m=c});j("unblock",function(){m=e});j("startAuto",function(){a[9]=e;A=N(a[10])});j("stopAuto",function(){a[9]=c;clearTimeout(A);S=c});j("adjust",function(){T(g,0);U()});j("getValue",function(a){return"currentSlide"==a?g+1:"totalSlides"==
a?k:"clickable"==a?m:"destroyed"==a?z:"autoAnimation"==a?S:Z})})}})(jQuery);